...
public class guardar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        EtiquetaBL etiqueta = new EtiquetaBL(Integer.parseInt(request.getParameter("id")), dbProperties);

        etiqueta.setNombre(request.getParameter("nombre"));
        PrintWriter out = response.getWriter();
        try {
            String msg;
            if (etiqueta.getId() == 0) {//Insertar
                etiqueta.insertar();
                msg = "Etiqueta insertado con éxito";
            } else { //Actualizar
                etiqueta.actualizar();
                msg = "Etiqueta actualizado con éxito";
            }
